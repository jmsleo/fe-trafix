...
export default function VehicleTypePage() {
  ...
  const { data, isLoading, isError, refetch } = useVehicleTypes({
  ...
  const handleKlikEdit = (vehicle: VehicleTypeRead) => {
  ...
  const handleSimpanVehicle = () => {
    ...
    if (!formData.code.trim()) {
      setFormError('Kode kendaraan wajib diisi.');
      return;
    }
      ...
      <div className="rounded-[10px] border border-[#B5884D] overflow-hidden shadow-lg bg-transparent w-full">
              ...
              {isLoading ? (
                ...
                items.map((v, index) => (
                  <tr key={v.id} className={`${index % 2 === 0 ? 'bg-[#322A1F]' : 'bg-[#231F1A]'} hover:bg-[#3d3326] transition-colors`}>
                    ...
                    <td className="px-6 py-4 text-center text-xs whitespace-nowrap">{formatTanggalUpdate(v.updated_at)}</td>
                    <td className="px-6 py-4 whitespace-nowrap">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between px-6 py-4 bg-transparent border-t border-[#B5884D]/30 gap-4 mt-auto">
          <span className="text-sm text-gray-400">
            {total === 0 ? 'Tidak ada data' : `Menampilkan ${startIndex}-${endIndex} dari ${total} Kendaraan`}
          </span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-[#17130E] border border-[#B5884D] w-full max-w-sm rounded-[10px] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <div className="flex justify-end gap-3 p-5 border-t border-[#B5884D]/30 bg-[#231F1A]">
              ...
              <Button onClick={handleSimpanVehicle} className="px-6 !h-auto py-2.5 !w-auto whitespace-nowrap" disabled={createVehicle.isPending || updateVehicle.isPending}>
                {(createVehicle.isPending || updateVehicle.isPending) ? 'Menyimpan...' : (idYangDiedit ? 'Simpan' : 'Tambah')}
              </Button>
            </div>
          </div>
        </div>

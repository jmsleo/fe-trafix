...
import {
  useCreateSignageAssignment,
  useDeleteSignageAssignment,
  useSignageAssignments,
  useSignageContents,
  useSignages,
  useUpdateSignageAssignmentStatus,
} from '@/hooks/useSignages';
import { getApiErrorMessage } from '@/lib/api/errors';
import type { SignageAssignmentRead } from '@/lib/api/types';
...
export default function AssignmentsPage() {
  ...
  const { data, isLoading, isError, refetch } = useSignageAssignments({
  ...
  const createAssignment = useCreateSignageAssignment();
  const deleteAssignment = useDeleteSignageAssignment();
  const updateStatus = useUpdateSignageAssignmentStatus();
  ...
  const handleSimpan = async () => {
    ...
    try {
      ...
      setFormError(getApiErrorMessage(error, 'Gagal menyimpan assignment. Coba lagi.'));
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-4 relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
        <h1 className="text-[32px] font-bold text-[#EAE1D8] whitespace-nowrap">Assignments</h1>

        <Button onClick={handleTambah} className="w-full md:w-auto md:px-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center p-4 rounded-[10px] border border-[#B5884D]/50 bg-transparent w-full gap-4">
        <div className="flex flex-wrap gap-4 w-full sm:w-auto">
          <div className="relative w-full sm:w-[180px]">
            <select
              value={signageFilter}
              onChange={(e) => { setSignageFilter(e.target.value); setPage(1); }}
      ...
      <div className="rounded-[10px] border border-[#B5884D] overflow-hidden shadow-lg bg-transparent w-full flex flex-col">
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-center text-[#EAE1D8]">
            ...
            <tbody>
              {isLoading ? (
                <tr><td colSpan={6} className="px-6 py-8 text-center text-gray-500 bg-[#231F1A]">Memuat data assignment...</td></tr>
              ) : isError ? (
                ...
                <tr><td colSpan={6} className="px-6 py-8 text-center text-gray-500 bg-[#231F1A]">Belum ada data assignment.</td></tr>
              ) : (
                items.map((a, index) => (
                  <tr key={a.id} className={`${index % 2 === 0 ? 'bg-[#322A1F]' : 'bg-[#231F1A]'} hover:bg-[#3d3326] transition-colors border-b border-[#B5884D]/10`}>
                    <td className="px-6 py-4 font-medium text-center whitespace-nowrap">{startIndex + index}.</td>
                    <td className="px-6 py-4 text-center whitespace-nowrap text-gray-300">{a.signage.name}</td>
                    <td className="px-6 py-4 text-center whitespace-nowrap text-gray-300">{a.content.title}</td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <button
                        onClick={() => handleToggleStatus(a)}
                        disabled={updateStatus.isPending}
                        className={`flex items-center justify-center mx-auto w-[97px] h-[23px] rounded-[9px] border text-[10px] font-semibold tracking-wide transition-colors disabled:opacity-50
                          ${a.is_active ? 'border-[#79FF8D] bg-[#00FF2659] text-[#79FF8D] hover:bg-[#00FF2680]' : 'border-[#FF8080] bg-[#FF000059] text-[#FF8080] hover:bg-[#FF000080]'}`}
                      >
                        {a.is_active ? 'AKTIF' : 'NON AKTIF'}
                      </button>
                    </td>
                    <td className="px-6 py-4 text-center text-xs whitespace-nowrap text-gray-400">
                      {formatTanggalUpdate(a.created_at)}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between px-6 py-4 bg-transparent border-t border-[#B5884D]/30 gap-4 mt-auto">
          <span className="text-sm text-gray-400">
            {total === 0 ? 'Tidak ada data' : `Menampilkan ${startIndex}-${endIndex} dari ${total} Assignment`}
          </span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-[#17130E] border border-[#B5884D] w-full max-w-sm rounded-[10px] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="flex justify-between items-center p-5 border-b border-[#B5884D]/30 bg-[#231F1A]">
              <h2 className="text-xl font-bold text-[#EAE1D8]">Tambah Assignment</h2>
              <button onClick={() => { setIsModalOpen(false); setSelectedSignageId(''); setSelectedContentId(''); setFormError(null); }} className="text-gray-400 hover:text-[#FF5656] transition-colors">
            ...
            <div className="p-6 space-y-5">
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <select value={selectedSignageId} onChange={(e) => setSelectedSignageId(e.target.value)} className="w-full appearance-none px-4 py-2.5 text-sm bg-black border border-[#B5884D]/50 rounded-[7px] text-[#EAE1D8] focus:outline-none focus:border-[#B5884D] cursor-pointer">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <select value={selectedContentId} onChange={(e) => setSelectedContentId(e.target.value)} className="w-full appearance-none px-4 py-2.5 text-sm bg-black border border-[#B5884D]/50 rounded-[7px] text-[#EAE1D8] focus:outline-none focus:border-[#B5884D] cursor-pointer">
            ...
            <div className="flex justify-end gap-3 p-5 border-t border-[#B5884D]/30 bg-[#231F1A]">
              <button onClick={() => { setIsModalOpen(false); setSelectedSignageId(''); setSelectedContentId(''); setFormError(null); }} className="px-6 py-2.5 text-sm font-medium text-[#EAE1D8] border border-gray-600 rounded-[7px] hover:bg-gray-800 transition-colors whitespace-nowrap">Batal</button>

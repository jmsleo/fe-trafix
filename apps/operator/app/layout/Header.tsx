...
interface HeaderProps {
  ...
  userName: string;
  userRole: string;
  avatarUrl?: string; // Tambahan untuk foto profil
  onLogout?: () => void;
}

export default function Header({ title, userName, userRole, avatarUrl, onLogout }: HeaderProps) {
  return (
    <header className="flex items-center justify-between pb-5 border-b-[2px] border-[#BF8F51] w-full">
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="flex items-center gap-3.5">
          <div className="flex items-center justify-center w-11 h-11 bg-[#EAEAEA] rounded-full text-black font-bold text-lg overflow-hidden">
            {/* Menampilkan foto jika ada, jika tidak tampilkan inisial */}
            {avatarUrl ? (
